var karbid = {
			...
			for(var i = 0;i<karbid.binder.bindings.length;i++){
				...
				if (binding.type == karbid.binder.BindingType.LOOPBINDING){
					...
					if (JSON.stringify(eval(binding.value)) != binding.previousValue){
						...
						document.qSelectorAll('[karbid-bindingid="'+binding.i+'"]').forEach(function(e,index){
								e.remove();
						})
	...
	renderLine:function (element,parent){
	...
	utils:{
		...
		toQuery:function(element,elementIndicator = true){
			var loop = "";
			var attributes = "";
			if (element.attributes.length>0){
				attributes = "[";
				element.attributes.forEach(function(e,i){
			...
			return (elementIndicator? "@" : "") + element.tag+(element.id != "" ? "#"+element.id : "")+(element.class!=""? "."+element.class.replace(" ",".") : "")+ attributes + loop+"{\n";
		},
		toQueryTree:function(element){
			var p = element;
			var ret = [];
			var elements = [];
			while (p!=undefined){
				ret.push(karbid.utils.toQuery(p));
				elements.push(p);
				p = p.parent;
			}
			if (ret.length == 1)
		...
		queryConverter: function(query) {
			var topmostElement = {id:"",tag:"",class:"",attributes:[],parent:undefined};
			var previousElement = topmostElement;
			for(var qIndex = query.split(">").length-1;qIndex>=0;qIndex--){
				var element = {id:"",tag:"",class:"",attributes:[],parent:undefined};
				var q = query.split(">")[qIndex];
				
				//loop
				if(q.includes(":") && q.includes("(") && q.includes(")")){ //checking if it contains loop
					var name = q.split("(")[1].split(":")[0];
					var to = eval(q.split(")")[0].split("(")[1].split(":")[1]);
					//var to = eval(karbid.utils.replaceAt(q.split(name+":")[1].split("{")[0],q.split(name+":")[1].split("{")[0].length-1,""));
					var array = new Array();
					var type = "";
					...
					element.loop = {
						...
						array:array,
						index:0,
						targetString:q.split(")")[0].split("(")[1].split(":")[1],
						binding:q.split("(")[0].endsWith("!"),
						varAsString:karbid.utils.replaceAt(q.substr(q.lastIndexOf(":")+1),q.substr(q.lastIndexOf(":")+1).length-1,"")
					}
				...
				for(var i=0;i<q.length;i++){
					...
					if (q[i] == "[") {q = karbid.utils.replaceAt(q,i,"");splitter = "[";element.attributes.push({name:"",value:""});}
					if (q[i] == "=") {q = karbid.utils.replaceAt(q,i,"");splitter = "=";}
					if (q[i] == "]") splitter = "]";
					if (q[i] == ",") {splitter = ",";element.attributes.push({name:"",value:""});};
					if(q[i] == "{") {break;}
					...
					if(splitter =="."){
						element.class+=q[i].replace("."," ");
					}
					if(splitter == "#"){
						element.id+=q[i].replace("#","");
					}
					//attributes
					if (splitter == "["){
						element.attributes[element.attributes.length-1].name +=q[i];
					}
					if (splitter == "="){
						element.attributes[element.attributes.length-1].value += q[i];
					}
					if (splitter == "]" || splitter==","){
						if (splitter ==",") splitter = "["; 
					}
					...
					/*if (splitter == ">"){
						element.parent.push({id:element.id,tag:element.tag,class:element.class,attributes:element.attributes,parent:[]});
						element.id = "";
						element.tag = "";
						element.class = "";
						element.attributes = [];
				...
				previousElement.parent = element;
				previousElement = element;
				if (query.split(">").length-1 == qIndex){
	...
	options:{
		Predefined:{
			...
			Attributes:[
				...
				{karbidName:"textAlign",attribute:"style.textAlign"}
			]
		},
		RenderOptions:{
